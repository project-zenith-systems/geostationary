...
| Module      | Description |
|-------------|-------------|
| `network`   | Transport and protocol layer: connection management, packet framing, and a typed message protocol for multiplayer state replication. Defines the wire format in domain-neutral terms — peers, spatial state, input vectors — while the upper layers decide what those primitives represent. The same way `physics` knows about rigid bodies and colliders without knowing they are creatures or walls, `network` knows about peers and positions without knowing they are players on a station. Must support the demands of a server-authoritative multiplayer simulation: many concurrent connections, frequent state updates, and both reliable (chat, interactions) and unreliable (movement, atmospherics sync) delivery. |
...
| `world`     | World lifecycle and map file infrastructure. Manages loading and tearing down the game world through lifecycle messages (`WorldLoading`, `WorldReady`, `WorldTeardown`). Owns the `.station.ron` map file container format and a `MapLayer` trait that higher modules implement to register their own serializable map data layers. The world module reads and writes map files, dispatching each named layer to its registered handler, but has no knowledge of what the layers contain — tiles, items, atmosphere, and structures are all opaque `ron::Value` blobs from this module's perspective. Preserves unrecognized layers on round-trip so maps authored with newer versions don't lose data. |
